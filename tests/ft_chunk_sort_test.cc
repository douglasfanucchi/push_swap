...
TEST(ft_push_range, itShouldPushToStackAOnlyElementsWithinTheRange) {
    t_stack *stacks[2];
    t_stack a;
    t_stack b;
    t_state state;
    ...
    ASSERT_EQ(3, a.size);
    ASSERT_EQ(3, b.size);
    ASSERT_EQ(5, ft_stack_peek(&a));
    ft_stack_rotate(&a);
    ASSERT_EQ(3, ft_stack_peek(&a));
    ft_stack_rotate(&a);
    ASSERT_EQ(4, ft_stack_peek(&a));
    ft_stack_clear(&a);
    ft_stack_clear(&b);
    ft_state_clear(&state);
}
...
TEST(ft_reverse_insertion_sort_substack, itShouldDoNothingWhenStackIsAlreadyReverseSorted) {
    t_stack a;
    t_stack b;
    t_state state;
    ft_stack_init(&a);
...
TEST(ft_reverse_insertion_sort_substack, itShouldSortInDescendingOrderSubstackWithSizeThreeAndTrashOnStackA) {
    t_stack a;
    t_stack b;
    t_state state;
    ft_stack_init(&a);
